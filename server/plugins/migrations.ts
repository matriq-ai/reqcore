...
export default defineNitroPlugin(async () => {
  ...
  if (process.env.RAILWAY_ENVIRONMENT_ID) {
    console.log('[Matriq] Skipping runtime migrations on Railway (handled in preDeploy)')
  ...
  try {
    ...
    if (!locked) {
      console.log('[Matriq] Another instance is running migrations, skipping')
      logInfo('migrations.skipped_locked')
      return
    }

    console.log('[Matriq] Running database migrations...')
    ...
    console.log('[Matriq] Database migrations applied successfully')
    logInfo('migrations.completed')
  } catch (error) {
    console.error('[Matriq] Migration failed:', error)
