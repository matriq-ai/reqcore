...
useSeoMeta({
  title: 'Localization Settings — Matriq',
  description: t('dashboard.settings.localization.seoDescription'),
})
...
async function handleSave() {
  ...
  try {
    ...
    saveError.value = err instanceof Error ? err.message : t('dashboard.settings.localization.saveFailed')
  }
  finally {
  ...
  <div class="mx-auto max-w-2xl">
    ...
    <div class="mb-6">
      <h1 class="text-lg font-semibold text-surface-900 dark:text-surface-50">
        {{ $t('dashboard.settings.localization.title') }}
      </h1>
      <p class="text-sm text-surface-500 dark:text-surface-400 mt-0.5">
        {{ $t('dashboard.settings.localization.subtitle') }}
      </p>
    </div>
    ...
    <section class="rounded-xl border border-surface-200 dark:border-surface-800 bg-white dark:bg-surface-900 overflow-hidden">
      <div class="px-4 sm:px-6 py-5 border-b border-surface-200 dark:border-surface-800">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="text-base font-semibold text-surface-900 dark:text-surface-100">{{ $t('dashboard.settings.localization.displayPreferences') }}</h2>
            <p class="text-sm text-surface-500 dark:text-surface-400">{{ $t('dashboard.settings.localization.displayPreferencesDescription') }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="px-4 sm:px-6 py-5 space-y-6">
        ...
        <div>
          <label class="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-2">
            {{ $t('dashboard.settings.localization.nameDisplayFormat') }}
          </label>
          <p class="text-xs text-surface-400 dark:text-surface-500 mb-3">
            {{ $t('dashboard.settings.localization.nameDisplayFormatDescription') }}
          </p>
          <div class="flex flex-col sm:flex-row gap-3">
            ...
            >
              ...
              />
              <div>
                <span class="block text-sm font-medium text-surface-800 dark:text-surface-200">{{ $t('dashboard.settings.localization.firstLast') }}</span>
                <span class="block text-xs text-surface-400 mt-0.5">{{ $t('dashboard.settings.localization.firstLastExample') }}</span>
              </div>
            </label>
            <label
              class="flex items-start gap-3 flex-1 rounded-lg border p-3.5 cursor-pointer transition-colors"
            ...
            >
              ...
              />
              <div>
                <span class="block text-sm font-medium text-surface-800 dark:text-surface-200">{{ $t('dashboard.settings.localization.lastFirst') }}</span>
                <span class="block text-xs text-surface-400 mt-0.5">{{ $t('dashboard.settings.localization.lastFirstExample') }}</span>
              </div>
            </label>
          </div>
        </div>
        ...
        <div>
          <label class="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-2">
            {{ $t('dashboard.settings.localization.dateFormat') }}
          </label>
          <p class="text-xs text-surface-400 dark:text-surface-500 mb-3">
            {{ $t('dashboard.settings.localization.dateFormatDescription') }}
          </p>
          <div class="flex flex-col sm:flex-row gap-3">
            <label
              class="flex items-start gap-3 flex-1 rounded-lg border p-3.5 cursor-pointer transition-colors"
            ...
            >
              ...
              <div>
                ...
                <span class="block text-xs text-surface-400 mt-0.5">{{ $t('dashboard.settings.localization.unitedStates') }}</span>
              </div>
            </label>
            <label
              class="flex items-start gap-3 flex-1 rounded-lg border p-3.5 cursor-pointer transition-colors"
            ...
            >
              ...
              <div>
                ...
                <span class="block text-xs text-surface-400 mt-0.5">{{ $t('dashboard.settings.localization.europeVietnamRegions') }}</span>
              </div>
            </label>
            <label
              class="flex items-start gap-3 flex-1 rounded-lg border p-3.5 cursor-pointer transition-colors"
            ...
            >
              ...
              <div>
                ...
                <span class="block text-xs text-surface-400 mt-0.5">{{ $t('dashboard.settings.localization.iso8601EastAsia') }}</span>
              </div>
            </label>
          </div>
        </div>
        ...
        <div class="rounded-lg border border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-800/50 p-4">
          <p class="text-xs font-medium text-surface-500 dark:text-surface-400 uppercase tracking-wide mb-2">{{ $t('dashboard.settings.localization.preview') }}</p>
          <div class="flex flex-col sm:flex-row sm:items-center gap-3 text-sm">
            <div class="flex items-center gap-2">
              <span class="text-surface-400">{{ $t('dashboard.settings.localization.previewName') }}</span>
            ...
            <div class="flex items-center gap-2">
              <span class="text-surface-400">{{ $t('dashboard.settings.localization.previewDateOfBirth') }}</span>
        ...
        <div class="flex items-center gap-3">
          ...
          >
            ...
            {{ saveSuccess ? $t('dashboard.settings.localization.saved') : isSaving ? $t('dashboard.settings.general.saving') : $t('dashboard.settings.general.saveChanges') }}
          </button>
          <p v-if="!canUpdateOrg" class="text-xs text-surface-400">{{ $t('dashboard.settings.localization.onlyAdminsCanChange') }}</p>
        </div>
      </div>
    </section>
  </div>
