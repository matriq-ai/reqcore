...
/**
 * Feishu (Lark) sign-up — global genericOAuth provider configured via env vars.
 * Redirects to Feishu's authorize page (offers QR-code scan login). New users
 * have no organization yet, so route them to org creation like social sign-up.
 */
async function handleFeishuSignUp() {
    isLoading.value = true;
    error.value = "";
    const callbackURL = pendingInvitation.value
        ? localePath(`/auth/accept-invitation/${pendingInvitation.value}`)
        ...
        <template v-if="feishuEnabled">
            ...
            >
                ...
                <template v-else>
                    ...
                    {{ $t('auth.signUp.continueWithProvider', { provider: feishuProviderName }) }}
                </template>
            </button>
